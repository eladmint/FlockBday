...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    // Set Twitter connection status from localStorage
    setIsTwitterConnected(
      localStorage.getItem("userTwitterConnected") === "true",
    );
    ...
    if (savedProfile) {
      // Use saved profile data if available
      setProfileData(JSON.parse(savedProfile));
    } else if (isDemoMode && currentUserId) {
      // In demo mode, load mock data
      setProfileData({
        name: currentUserId === "user123" ? "John Doe" : "Jane Smith",
        email:
          currentUserId === "user123" ? "john@example.com" : "jane@example.com",
        bio: "Marketing professional with experience in digital campaigns.",
        notifyEmails: true,
        notifyPosts: true,
        notifyMentions: true,
      });
    } else if (isLoaded && user) {
      // In real mode with a logged in user
      setProfileData({
        name: user.fullName || "",
  ...
  const handleDisconnectTwitter = async () => {
    ...
    try {
      ...
      localStorage.removeItem("userTwitterConnected");
  ...
  const userInfo = isDemoMode
    ? {
        name: currentUserId === "user123" ? "John Doe" : "Jane Smith",
        imageUrl: `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentUserId === "user123" ? "John" : "Jane"}`,
      }
    : {
        name: user?.fullName,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    disabled={!isDemoMode} // Only editable in demo mode
                  />
                </div>
